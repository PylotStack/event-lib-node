...
export declare enum ActionHandlerEnum {
    COMMIT = "COMMIT",
    REJECT = "REJECT"
}
...
export declare type ActionHandler = (context: ActionHandlerContext, payload: any) => ActionHandlerResult;
export declare type AsyncActionHandler = (context: ActionHandlerContext, payload: any) => Promise<ActionHandlerResult>;
export declare type ViewEventBuilderHandler<ViewType = any> = (state: ViewType, ev: ESEvent) => any;
export declare type ViewFinalizerHandler<ViewType = any> = (state: ViewType) => any;
...
export declare type MapAction<U extends string> = <T extends (...args: any) => any>(actionType: U, handler: T) => ((...args: Parameters<T>) => Promise<void>);
export declare type MapView = (<T>(viewDefinition: ViewDefinition<T>) => T) & (<T, U extends keyof T>(viewDefinition: ViewDefinition<T>, property: U) => T[U]) & (<T, K, U extends (view: T) => any>(viewDefinition: ViewDefinition<T>, transformer: U) => ReturnType<U>);
